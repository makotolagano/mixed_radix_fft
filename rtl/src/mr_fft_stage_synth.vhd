...
entity mr_fft_stage_synth is
  port (
    ...
    i_config_delay : in std_logic_vector(clogb2(get_delay_cnt(c_slot_cfgs(3))) - 1 downto 0);
    i_sample : in  t_cmplx;
...
architecture rtl of mr_fft_stage_synth is
  constant C_SLOT : natural := 3;  -- first radix235 slot
begin
  dut : entity work.mr_fft_stage
    generic map (G_CAPABILITY => f_slot_capability(C_SLOT), G_CONFIGS => c_slot_cfgs(C_SLOT), G_TWIDDLE_ROM_STYLE => "auto")
    port map (
